...
import ora from "ora";
import MONGO_URIS from "../jsons/mongo_uris.json" assert { type: "json" };
...
const backupDatabase = async (log, { uri, name }, useSpinner = true) => {
  log(`Backing up ${name}...`);
...
const runBackupsInBatches = async (log, batchSize = 2) => {
  ...
  for (let i = 0; i < MONGO_URIS.length; i += batchSize) {
    ...
    const results = await Promise.all(
      batch.map((db) =>
        backupDatabase(log, db, false).catch(({ err, duration }) => {
...
export const runBackup = async () => {
  ...
  try {
    await runBackupsInBatches(log, CONCURRENT);
  } finally {
    logStream.end();
  }
};
